  ...
  async init () {
    ...
    return {
      callback () {
        ...
        $id(tool_id).setAttribute("title", `${sport} Positions`);
        $id(tool_id).setAttribute("src", `../cs-drill-editor/tool-extensions/ext-cs-positions/tool_button_position.svg`);
        $id(tool_id).setAttribute("lib", `${extPath}/ext-cs-positions/`);
      ...
      mouseDown (opts) {
        ...
        if (mode !== modeId) {
          // console.log(`mouseDown:unexpected mode\nexpecting ${modeId} got ${mode}`)
          return undefined
        }

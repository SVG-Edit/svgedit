...
export default {
  ...
  async init() {
    ...
    return {
      ...
      callback() {
        const title = svgEditor.i18next.t(`${name}:${sport}.title`);
        ...
        if ($id(idButton) === null) {
          ...
          <se-button id="${idButton}" src="${imgButton}"></se-button>
          `
        ...
        //set sport specific hover text
        $id(idButton).setAttribute("title", title);

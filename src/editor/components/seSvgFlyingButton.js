 ...
 */
export class SvgFlyingButton extends HTMLElement {
  /**
...
<!--     </div>-->
  ...
  static get observedAttributes () {
    return ['sport', 'title', 'pressed', 'disabled', 'lib', 'src']
  }
  ...
  async attributeChangedCallback (name, oldValue, newValue) {
    ...
    switch (name) {
      ...
      case 'title':
        {
          let shortcut = this.getAttribute('shortcut')
  ...
  connectedCallback () {
    ...
    const onClickHandler = (ev) => {
      ...
      switch (ev.target.nodeName) {
        case 'SE-SVGFLYINGBUTTON':
          //this.$menu.classList.toggle('open')
          this.$lib.classList.toggle('open-lib')

          const rect = this.getBoundingClientRect()
          this.$lib.style.top = rect.top + 'px'
  ...
  async updateLib (lib) {
    ...
    try {
      ...
      for (let i = 0; i < lib.length; i++) {
        ...
        innerHTML += `<se-button data-shape="${s.id}" title="${s.title}" src="data:image/svg+xml;base64,${btn}"></se-button>`
      }
      this.$lib.innerHTML = innerHTML
      ...
      console.error(`could not read file:${libDir}${path}`, error)
    }
  }
}

// Register
customElements.define('se-svgflyingbutton', SvgFlyingButton)

...
export default class ConfigObj {
    ...
    if (search) {
      ...
      ;['initStroke', 'initFill'].forEach(prop => {
        if (searchParams.has(`${prop}[color]`)) {
          ...
          if (this.urldata[prop] === undefined) {
            this.urldata[prop] = {}
          }
      ...
      ;['userExtensions', 'imgPath'].forEach(function (pathConfig) {
    ...
    Object.keys(this.defaultPrefs).forEach(key => {
      ...
      if (this.editor.storage) {
        ...
        const result = document.cookie.match(
          new RegExp(
            '(?:^|;\\s*)' +
              regexEscape(encodeURIComponent(storeKey)) +
              '=([^;]+)'
          )
        )
  ...
  setConfig (opts, cfgCfg = {}) {
    ...
    const extendOrAdd = (cfgObj, key, val) => {
      if (
        cfgObj[key] &&
        typeof cfgObj[key] === 'object' &&
        !Array.isArray(cfgObj[key])
      ) {
    ...
    Object.entries(opts).forEach(([key, val]) => {
      ...
      if (this.defaultPrefs[key]) {
        if (
          ...
          (this.curConfig.preventAllURLConfig || this.curPrefs[key])
        ) {
          return
        }
        ...
        ['extensions', 'userExtensions', 'allowedOrigins'].includes(key)
      ) {
        if (
          cfgCfg.overwrite === false &&
        ...
        if ({}.hasOwnProperty.call(this.curConfig, key)) {
          ...
          typeof this.defaultConfig[key] === 'object' &&
          !Array.isArray(this.defaultConfig[key])
        ) {
          this.curConfig[key] = {}

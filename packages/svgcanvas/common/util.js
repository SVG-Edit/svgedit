...
export function getClosest (elem, selector) {
  ...
  for (; elem && elem !== document && elem.nodeType === 1; elem = elem.parentNode) {
    ...
    if (firstChar === '.') {
      if (supports) {
        if (elem.classList.contains(selector.slice(1))) {
          return elem
        }
      } else {
        if (new RegExp(`(^|\\s)${selector.slice(1)}(\\s|$)`).test(elem.className)) {
          return elem
        }
      }
    }
...
export function getParents (elem, selector) {
  ...
  for (; elem && elem !== document; elem = elem.parentNode) {
    if (selector) {
      ...
      // If selector is a data attribute
      if (firstChar === '[') {
        if (elem.hasAttribute(selector.slice(1, -1))) {
          parents.push(elem)
        }
      }
      // If selector is a tag
...
export function getParentsUntil (elem, parent, selector) {
  ...
  for (; elem && elem !== document; elem = elem.parentNode) {
    ...
    if (parent) {
      ...
      if (parentType === '.') {
        if (elem.classList.contains(parent.slice(1))) {
          break
        }
      }
      // If parent is an ID
      if (parentType === '#') {
        if (elem.id === parent.slice(1)) {
          break
        }
      }
      // If parent is a data attribute
      if (parentType === '[') {
        if (elem.hasAttribute(parent.slice(1, -1))) {
          break
        }
      }
      // If parent is a tag

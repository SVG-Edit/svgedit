var SvgCanvas = (function () {
  ...
  function jQueryPluginSVG($) {
    ...
    */
    ...
    $.fn.attr = function (key, value) {
      ...
      for (let i = 0; i < len; ++i) {
        const elem = this[i]; // set/get SVG attribute
        ...
        if (elem.namespaceURI === svgns) {
          ...
          if (value !== undefined) {
            ...
            while (j--) {
              const aname = key[j];
              let attr = elem.getAttribute(aname); // This returns a number when appropriate
  ...
  */
  ...
  const svg = document.createElementNS(NS.SVG, 'svg'); // Note: Browser sniffing should only be used if no other detection method is possible
  ...
  */
  ...
  const isChrome = () => isChrome_;
  /**
   * @function module:browser.isTouch
   * @returns {boolean}
  */

  const isTouch = () => isTouch_;
  /**
   * @function module:browser.supportsSelectors
   * @returns {boolean}
  */

  const supportsSelectors = () => supportsSelectors_;
  /**
   * @function module:browser.supportsXpath
   * @returns {boolean}
  */

  const supportsXpath = () => supportsXpath_;
  /**
   * @function module:browser.supportsPathReplaceItem
   * @returns {boolean}
  */

  const supportsPathReplaceItem = () => supportsPathReplaceItem_;
  /**
   * @function module:browser.supportsPathInsertItemBefore
   * @returns {boolean}
  */

  const supportsPathInsertItemBefore = () => supportsPathInsertItemBefore_;
  /**
   * @function module:browser.supportsPathBBox
   * @returns {boolean}
  */

  const supportsPathBBox = () => supportsPathBBox_;
  /**
   * @function module:browser.supportsHVLineContainerBBox
   * @returns {boolean}
  */

  const supportsHVLineContainerBBox = () => supportsHVLineContainerBBox_;
  /**
  ...
  */
  ...
  const supportsNonScalingStroke = () => supportsNonScalingStroke_;
  /**
  * @function module:browser.supportsNativeTransformLists
  * @returns {boolean}
  */

  const supportsNativeTransformLists = () => supportsNativeSVGTransformLists_;
      ...
      case 4:
        {
          // ROTATE
          let cx = 0;
          let cy = 0; // this prevents divide by zero
    ...
    */
    constructor(elem) {
      ...
      this._update = function () {
        let tstr = ''; // /* const concatMatrix = */ svgroot.createSVGMatrix();
      ...
      this._init = function () {
        ...
        const re = /\s*((scale|matrix|rotate|translate)\s*\(.*?\))\s*,?\s*/; // const re = /\s*(?<xform>(?:scale|matrix|rotate|translate)\s*\(.*?\))\s*,?\s*/;
    ...
    insertItemBefore(newItem, index) {
      ...
      if (index >= 0) {
        if (index < this.numberOfItems) {
          ...
          const newxforms = new Array(this.numberOfItems + 1); // TODO: use array copying and slicing
  ...
  */
  ...
  let removeElementFromListMap = function (elem) {
  ...
  */
  ...
  const transformListToTransform = function (tlist, min, max) {
    ...
    let m = svg$1.createSVGMatrix();

    for (let i = min; i <= max; ++i) {
      // if our indices are out of range, just use a harmless identity matrix
      const mtom = i >= 0 && i < tlist.numberOfItems ? tlist.getItem(i).matrix : svg$1.createSVGMatrix();
  ...
  */
  ...
  const snapToAngle = function (x1, y1, x2, y2) {
  ...
  const $$1 = jQueryPluginSVG(jQuery); // String used to encode base64.

  const KEYSTR = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/='; // Much faster than running getBBox() every time

  const visElems = 'a,circle,ellipse,foreignObject,g,image,line,path,polygon,polyline,rect,svg,text,tspan,use,clipPath';
  const visElemsArr = visElems.split(','); // const hidElems = 'defs,desc,feGaussianBlur,filter,linearGradient,marker,mask,metadata,pattern,radialGradient,stop,switch,symbol,title,textPath';
  ...
  */
  ...
  const init = function (editorContext) {
  ...
  const dropXMLInternalSubset = str => {
  ...
  */
  ...
  const encodeUTF8 = function (argString) {
  ...
  const dataURLToObjectURL = function (dataurl) {
  ...
  */
  ...
  const walkTreePost = function (elem, cbFn) {
    if (elem && elem.nodeType === 1) {
      let i = elem.childNodes.length;
  ...
  */
  ...
  const getUrlFromAttr = function (attrVal) {
    if (attrVal) {
      // url('#somegrad')
  ...
  */
  ...
  const getPathBBox = function (path) {
    const seglist = path.pathSegList;
    ...
    const getCalc = function (j, P1, P2, P3) {
      return function (t) {
        return 1 - t ** 3 * P0[j] + 3 * 1 - t ** 2 * t * P1[j] + 3 * (1 - t) * t ** 2 * P2[j] + t ** 3 * P3[j];
      };
    };
  ...
  */
  ...
  function groupBBFix(selected) {
    ...
    if (matched.length) {
      ...
      if (issue) {
        const elems = ref ? copy : $$1(selected).children();
      ...
      default:
        ...
        if (elname === 'use' || elname === 'foreignObject' && isWebkit()) {
          ...
          if (selected) {
            ...
            const fo = $$1(selected).closest('foreignObject');
  ...
  */
  ...
  const getPathDFromSegments = function (pathSegments) {
    let d = '';
    $$1.each(pathSegments, function (j, [singleChar, pts]) {
      d += singleChar;
  ...
  */
  ...
  const getExtraAttributesForConvertToPath = function (elem) {
    const attrs = {}; // TODO: make this list global so that we can properly maintain it
  ...
  */
  ...
  const convertToPath = function (elem, attrs, addSVGElementFromJson, pathActions, clearSelection, addToSelection, hstry, addCommandToHistory) {
    const batchCmd = new hstry.BatchCommand('Convert element to Path'); // Any attribute on the element not covered by the passed-in attributes
  ...
  */
  ...
  const getStrokedBBox = function (elems, addSVGElementFromJson, pathActions) {
    ...
    let minX = fullBb.x;
    let minY = fullBb.y; // If only one elem, don't call the potentially slow getBBoxWithTransform method again.
    ...
    if (elems.length === 1) {
      ...
      $$1.each(elems, function (i, elem) {
        ...
        if (curBb) {
          ...
          minY = Math.min(minY, curBb.y - offset); // TODO: The old code had this test for max, but not min. I suspect this test should be for both min and max
  ...
  */
  ...
  const getStrokedBBoxDefaultVisible = function (elems) {
    if (!elems) {
  ...
  */
  ...
  let getRotationAngle = function (elem, toRad) {
    // eslint-disable-line import/no-mutable-exports
    const selected = elem || editorContext_.getSelectedElements()[0]; // find the rotation transform (if any) and set it
  ...
  */
  ...
  const getRefElem = function (attrVal) {
  ...
  */
  ...
  const getElem = supportsSelectors() ? function (id) {
  ...
  */
  ...
  const assignAttributes = function (elem, attrs, suspendLength, unitCheck) {
  ...
  */
  ...
  const cleanupElement = function (element) {
    const defaults = {
      'fill-opacity': 1,
      'stop-opacity': 1,
      opacity: 1,
  ...
  */
  ...
  const preventClickDefault = function (img) {
  ...
  */
  ...
  const init$1 = function (elementContainer) {
  ...
  */
  ...
  const convertToNum = function (attr, val) {
    ...
    const num = val.substr(0, val.length - 2); // Note that this multiplication turns the string into a number
  ...
  class RemoveElementCommand extends Command {
    ...
    */
    constructor(elem, oldNextSibling, oldParent, text) {
      ...
      this.parent = oldParent; // special hack for webkit: remove this element's entry in the svgTransformLists map
  ...
  */
  ...
  const setUiStrings = function (strs) {
    Object.assign(uiStrings, strs.ui);
  };
  let pathFuncs = [];
  let linkControlPts = true; // Stores references to paths via IDs.
  ...
  */
  ...
  const setLinkControlPoints = function (lcp) {
    linkControlPts = lcp;
  };
  /**
  ...
  */
  ...
  let path = null; // eslint-disable-line import/no-mutable-exports

  let editorContext_$1 = null;
  /**
  * @external MouseEvent
  */
  ...
  */
  ...
  const init$2 = function (editorContext) {
  ...
  */
  ...
  const insertItemBefore = function (elem, newseg, index) {
  ...
  */
  ...
  const ptObjToArr = function (type, segItem) {
  ...
  */
  ...
  const getGripPt = function (seg, altPt) {
    const {
      path: pth
    } = seg;
    let out = {
    ...
    const currentZoom = editorContext_$1.getCurrentZoom();
    out.x *= currentZoom;
    out.y *= currentZoom;
    return out;
  };
  ...
  */
  ...
  const getPointFromGrip = function (pt, pth) {
    const out = {
      x: pt.x,
      y: pt.y
    };
  ...
  */
  ...
  const addPointGrip = function (index, x, y) {
    // create the container of all the point grips
    const pointGripContainer = getGripContainer();
    let pointGrip = getElem('pathpointgrip_' + index); // create it
    ...
    if (!pointGrip) {
      ...
      const grip = $$2('#pathpointgrip_' + index);
  ...
  */
  ...
  const getGripContainer = function () {
  ...
  */
  ...
  const getPointGrip = function (seg, update) {
    const {
      index
    } = seg;
  ...
  */
  ...
  const getControlPoints = function (seg) {
    const {
      item,
      index
    } = seg;
  ...
  */
  ...
  const replacePathSeg = function (type, index, pts, elem) {
  ...
  */
  ...
  const getSegSelector = function (seg, update) {
    const {
      index
    } = seg;
  ...
  class Path {
    ...
    */
    ...
    setSegType(newType) {
      ...
      while (i--) {
        const selPt = this.selected_pts[i]; // Selected seg
          ...
          case 6:
            {
              if (cur.olditem) {
                ...
                const diffY = curY - prevY; // get control points from straight line segment
    ...
    */
    ...
    static subpathIsClosed(index) {
      let clsd = false; // Check if subpath is already open
  ...
  */
  ...
  const getRotVals = function (x, y) {
    let dx = x - oldcx;
    let dy = y - oldcy; // rotate the point around the old center
  ...
  */
  ...
  const recalcRotatedPath = function () {
    ...
    const Rnc = editorContext_$1.getSVGRoot().createSVGTransform(),
  ...
  */
  ...
  const reorientGrads = function (elem, m) {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (attrVal && attrVal.startsWith('url(')) {
        ...
        if (grad.tagName === 'linearGradient') {
          ...
          let y2 = grad.getAttribute('y2') || 0; // Convert to USOU points
          ...
          const pt2 = transformPoint(x2, y2, m); // Convert back to BB points
  ...
  */
  ...
  const convertPath = function (pth, toRel) {
    const {
      pathSegList
    } = pth;
    ...
    for (let i = 0; i < len; ++i) {
      const seg = pathSegList.getItem(i); // if these properties are not in the segment, set them to zero
  ...
  */
  ...
  const pathActions = function () {
    /* eslint-enable jsdoc/require-property */
    let subpath = false;
    let newPoint, firstCtrl;
    let currentPath = null;
    let hasMoved = false; // No `editorContext_` yet but should be ok as is `null` by default
      ...
      /** @lends module:path.pathActions */
      {
        ...
        */
        mouseDown(evt, mouseTarget, startX, startY) {
          ...
          if (editorContext_$1.getCurrentMode() === 'path') {
            ...
            let keep = null;
            let index; // if pts array is empty, create path element with M at current point

            const drawnPath = editorContext_$1.getDrawnPath();
            ...
            if (!drawnPath) {
              ...
              let newseg;
              let sSeg;
              const len = seglist.numberOfItems; // if we clicked on an existing point, then we are done this path, commit it
              ...
              if (clickOnPoint) {
                ...
                if (subpath) {
                  ...
                  const origD = $$2(path.elem).attr('d');
          ...
          if (id.substr(0, 14) === 'pathpointgrip_') {
            ...
            const seg = path.segs[curPt]; // only clear selection if shift is not pressed (otherwise, add
            // node to selection)
          ...
          if (!path.dragging) {
            let rubberBox = editorContext_$1.getRubberBox();
        ...
        mouseMove(mouseX, mouseY) {
          ...
          if (path.dragging) {
            ...
            path.eachSeg(function (i) {
              ...
              const rubberBox = editorContext_$1.getRubberBox();
        ...
        */
        mouseUp(evt, element, mouseX, mouseY) {
          const drawnPath = editorContext_$1.getDrawnPath(); // Create mode
        ...
        */
        reorient() {
          const elem = editorContext_$1.getSelectedElements()[0];
        ...
        */
        clear(remove) {
          ...
          if (drawnPath) {
            const elem = getElem(editorContext_$1.getId());
        ...
        */
        resetOrientation(pth) {
          ...
          const segList = pth.pathSegList; // Opera/win/non-EN throws an error here.
          // TODO: Find out why!
        ...
        */
        clonePathNode() {
          path.storeD();
          const selPts = path.selected_pts; // const {segs} = path;
        ...
        */
        opencloseSubPath() {
          const selPts = path.selected_pts; // Only allow one selected node for now
          ...
          const list = elem.pathSegList; // const len = list.numberOfItems;

          const index = selPts[0];
          let openPt = null;
          let startItem = null; // Check if subpath is already open
          ...
          let lastM, zSeg; // Find this sub-path's closing point and remove
          ...
          const pt = list.getItem(lastM); // Make this point the new "M"
          ...
          const cleanup = function () {
            ...
            while (len--) {
              ...
              if (item.pathSegType === 1) {
                ...
                if (len > 0) {
                  const prevType = segList.getItem(len - 1).pathSegType; // Path has M M
  ...
  var pathModule = /*#__PURE__*/Object.freeze({
    ...
    setLinkControlPoints: setLinkControlPoints,
    ...
    addPointGrip: addPointGrip,
    getGripContainer: getGripContainer,
    ...
    Path: Path,
    getPath_: getPath_,
    removePath_: removePath_,
    recalcRotatedPath: recalcRotatedPath,
    clearData: clearData,
    reorientGrads: reorientGrads,
  ...
  const $$4 = jQueryPluginSVG(jQuery);
  /**
  ...
  const copyElem = function (el, getNextId) {
  ...
  /* globals jQuery */
  const $$5 = jQuery;
  const visElems$1 = 'a,circle,ellipse,foreignObject,g,image,line,path,polygon,polyline,rect,svg,text,tspan,use'.split(',');
  const RandomizeModes = {
    LET_DOCUMENT_DECIDE: 0,
    ALWAYS_RANDOMIZE: 1,
    NEVER_RANDOMIZE: 2
  };
  let randIds = RandomizeModes.LET_DOCUMENT_DECIDE; // Array with current disabled elements (for in-group editing)
  ...
  function getNewLayerName(existingLayerNames) {
    let i = 1; // TODO(codedread): What about internationalization of "Layer"?
  ...
  class Drawing {
    ...
    */
    constructor(svgElem, optIdPrefix) {
      ...
      */
      ...
      this.nonce_ = '';
      const n = this.svgElem_.getAttributeNS(NS.SE, 'nonce'); // If already set in the DOM, use the nonce throughout the document
    ...
    */
    ...
    getNextId() {
      ...
      let restoreOldObjNum = false; // If there are any released numbers in the release stack,
    ...
    */
    ...
    mergeLayer(hrService) {
      const currentGroup = this.current_layer.getGroup();
      const prevGroup = $$5(currentGroup).prev()[0];
    ...
    */
    ...
    identifyLayers() {
      this.all_layers = [];
      this.layer_map = {};
      const numchildren = this.svgElem_.childNodes.length; // loop through all children of SVG element
      ...
      for (let i = 0; i < numchildren; ++i) {
        const child = this.svgElem_.childNodes.item(i); // for each g, find its layer name
    ...
    */
    ...
    cloneLayer(name, hrService) {
      ...
      const group = layer.getGroup(); // Clone children
  ...
  const randomizeIds = function (enableRandomization, currentDrawing) {
  ...
  */
  ...
  const createLayer = function (name, hrService) {
    const newLayer = canvas_.getCurrentDrawing().createLayer(name, historyRecordingService(hrService));
  ...
  const cloneLayer = function (name, hrService) {
  ...
  */
  ...
  const deleteCurrentLayer = function () {
    ...
    if (currentLayer) {
      const batchCmd = new BatchCommand('Delete Layer'); // store in our Undo History
  ...
  */
  ...
  const setCurrentLayer = function (name) {
  ...
  */
  ...
  const renameCurrentLayer = function (newName) {
  ...
  */
  ...
  const setCurrentLayerPosition = function (newPos) {
    const drawing = canvas_.getCurrentDrawing();
  ...
  */
  ...
  const setLayerVisibility = function (layerName, bVisible) {
    const drawing = canvas_.getCurrentDrawing();
  ...
  */
  ...
  const moveSelectedToLayer = function (layerName) {
    // find the layer
    const drawing = canvas_.getCurrentDrawing();
    ...
    const batchCmd = new BatchCommand('Move Elements to Layer'); // loop for each selected element and move it
    ...
    while (i--) {
      ...
      const oldNextSibling = elem.nextSibling; // TODO: this is pretty brittle!
  ...
  */
  ...
  const mergeLayer = function (hrService) {
  ...
  */
  ...
  const mergeAllLayers = function (hrService) {
  ...
  */
  ...
  const setContext = function (elem) {
    leaveContext();
    ...
    $$5(elem).parentsUntil('#svgcontent').andSelf().siblings().each(function () {
      const opac = this.getAttribute('opacity') || 1; // Store the original's opacity
  ...
  */
  ...
  const sanitizeSvg = function (node) {
    // Cleanup text nodes
    if (node.nodeType === 3) {
      // 3 === TEXT_NODE
      // Trim whitespace
    ...
    const parent = node.parentNode; // can parent ever be null here?  I think the root node's parent is the document...
    ...
    const allowedAttrsNS = svgWhiteListNS_[node.nodeName]; // if this element is supported, sanitize it
    ...
    if (typeof allowedAttrs !== 'undefined') {
      ...
      while (i--) {
        ...
        const attrNsURI = attr.namespaceURI; // Check that an attribute with the correct localName in the correct namespace is on
        ...
        if (isGecko()) {
          switch (attrName) {
            ...
            case 'patternTransform':
              {
                const val = attr.value.replace(/(\d)-/g, '$1 -'); // const val = attr.value.replace(/(?<digit>\d)-/g, '$<digit> -');
        ...
        if (attrName === 'style') {
          ...
          while (p--) {
            ...
            const styleAttrVal = (val || '').trim(); // Now check that this attribute is supported
  ...
  */
  ...
  async function importSetGlobal(url, {
    global: glob,
    returnDefault
  }) {
    ...
    if (modularVersion) {
      return importModule(url, undefined, {
        returnDefault
      });
    }
  ...
  */
  ...
  function importModule(url, atts = {}, {
    returnDefault = false
  } = {}) {
    if (Array.isArray(url)) {
      return Promise.all(url.map(u => {
  ...
  /* globals jQuery */
  const $$6 = jQuery; // this is how we map paths to our preferred relative segment types
  ...
  */
  ...
  const init$4 = function (editorContext) {
  ...
  */
  ...
  const remapElement = function (selected, changes, m) {
    const remap = function (x, y) {
    ...
    if (elName === 'g' || elName === 'text' || elName === 'tspan' || elName === 'use') {
      ...
      if (m.a === 1 && m.b === 0 && m.c === 0 && m.d === 1 && (m.e !== 0 || m.f !== 0)) {
        ...
        const mt = editorContext_$2.getSVGRoot().createSVGTransform();
    ...
    switch (elName) {
      ...
      case 'circle':
        {
          const c = remap(changes.cx, changes.cy);
          changes.cx = c.x;
          changes.cy = c.y; // take the minimum of the new selected box's dimensions for the new circle radius
      ...
      case 'g':
        {
          const gsvg = $$6(selected).data('gsvg');
      ...
      case 'path':
        {
          ...
          for (let i = 1; i < len; ++i) {
            ...
            const {
              type
            } = seg; // if absolute or first segment, we want to remap x, y, x1, y1, x2, y2
            // if relative, we want to scalew, scaleh

            if (type % 2 === 0) {
              // absolute
              const thisx = seg.x !== undefined ? seg.x : currentpt.x,
                    // for V commands
  ...
  /* globals jQuery */
  const $$7 = jQueryPluginSVG(jQuery);
  let context_;
  /**
  ...
  */
  ...
  const init$5 = function (editorContext) {
  ...
  */
  ...
  const updateClipPath = function (attr, tx, ty) {
  ...
  */
  ...
  const recalculateDimensions = function (selected) {
    ...
    const tlist = getTransformList(selected); // remove any unnecessary transforms
    ...
    if (tlist) {
      ...
      if (k >= 2 && tlist.getItem(k - 2).type === 1 && tlist.getItem(k - 1).type === 2) {
        ...
        const m = matrixMultiply(tlist.getItem(k - 2).matrix, tlist.getItem(k - 1).matrix);
        mt.setMatrix(m);
    ...
    const gsvg = $$7(selected).data('gsvg'); // we know we have some transforms, so set up return variable

    const batchCmd = new BatchCommand('Transform'); // store initial values that will be affected by reducing the transform list
    ...
    let oldcenter, newcenter; // if it's a regular group, we have special processing to flatten transforms
    ...
    if (selected.tagName === 'g' && !gsvg || selected.tagName === 'a') {
      ...
      let oldStartTransform; // first, if it was a scale then the second-last transform will be it
      ...
      if (N >= 3 && tlist.getItem(N - 2).type === 3 && tlist.getItem(N - 3).type === 2 && tlist.getItem(N - 1).type === 2) {
        ...
        while (c--) {
          ...
          if (child.nodeType === 1) {
            const childTlist = getTransformList(child); // some children might not have a transform (<metadata>, <defs>, etc)

            if (!childTlist) {
              continue;
            }

            const m = transformListToTransform(childTlist).matrix; // Convert a matrix to a scale if applicable
        ...
        e2t.setMatrix(m);
        tlist.clear();
        tlist.appendItem(e2t); // next, check if the first transform was a translate
          ...
          while (c--) {
            ...
            if (child.nodeType === 1) {
              ...
              if (childTlist) {
                ...
                batchCmd.addSubCommand(recalculateDimensions(child)); // If any <use> have this group as a parent and are
      ...
      if (angle) {
        ...
        const s = Math.abs(a) > 1.0e-10 ? Math.sin(a) / (1 - Math.cos(a)) // TODO: This blows up if the angle is exactly 0!
        : 2 / a;
      ...
      let operation = 0;
      const N = tlist.numberOfItems; // Check if it has a gradient with userSpaceOnUse, in which case
      ...
      if (N >= 3 && tlist.getItem(N - 2).type === 3 && tlist.getItem(N - 3).type === 2 && tlist.getItem(N - 1).type === 2) {
        ...
        tlist.removeItem(N - 1);
        tlist.removeItem(N - 2);
        tlist.removeItem(N - 3); // if we had [T][S][-T][M], then this was a skewed element being resized
        ...
        e2t.setMatrix(m);
        tlist.clear();
        tlist.appendItem(e2t); // reset the matrix so that the element is not re-mapped
  ...
  class Selector {
    ...
    */
    ...
    resize(bbox) {
      ...
      const tlist = getTransformList(selected);
      const m = transformListToTransform(tlist).matrix; // This should probably be handled somewhere else, but for now
  ...
  */
  ...
  class SelectorManager {
    ...
    */
    ...
    requestSelector(elem, bbox) {
      ...
      const N = this.selectors.length; // If we've already acquired one for this element, return it.
  ...
  const init$6 = function (config, svgFactory) {
    config_ = config;
  ...
  class SvgCanvas {
    ...
    */
    constructor(container, config) {
      ...
      const svgdoc = container.ownerDocument; // This is a container for the document being edited, not the document itself.
      ...
      */
      ...
      let currentZoom = 1; // pointer to current group (for in-group editing)

      let currentGroup = null; // Object containing data for the currently selected styles
      ...
      const curShape = allProperties.shape; // Array with all the currently selected elements
      ...
      */
      ...
      const addSVGElementFromJson = this.addSVGElementFromJson = function (data) {
        ...
        let shape = getElem(data.attr.id); // if shape is a path but we need to create a rect/ellipse, then remove the path
      ...
      */
      ...
      const undoMgr = canvas.undoMgr = new UndoManager$1({
        /**
         * @param {string} eventType One of the HistoryEvent types
        ...
        handleHistoryEvent(eventType, cmd) {
          const EventTypes = HistoryEventTypes$1; // TODO: handle setBlurOffsets.
          ...
          if (eventType === EventTypes.BEFORE_UNAPPLY || eventType === EventTypes.BEFORE_APPLY) {
            ...
            if (cmdType === 'MoveElementCommand') {
              ...
              const values = isApply ? cmd.newValues : cmd.oldValues; // If stdDeviation was changed, update the blur.
      ...
      */
      ...
      const getMouseTarget = this.getMouseTarget = function (evt) {
        ...
        let mouseTarget = evt.target; // if it was a <use>, Opera and WebKit return the SVGElementInstance
        ...
        const $target = $$9(mouseTarget); // If it's a selection grip, return the grip parent
      ...
      */
      ...
      const elData = $$9.data; // Animation element to change the opacity of any newly created element
      ...
      const restoreRefElems = function (elem) {
        ...
        const attrs = $$9(elem).attr(refAttrs);
      ...
      */
      ...
      this.addExtension = async function (name, extInitFunc, {
        ...
        const argObj = $$9.extend(canvas.getPrivateMethods(), {
          $: jq,
          importLocale,
          svgroot,
          svgcontent,
      ...
      */
      ...
      this.setRotationAngle = function (val, preventUndo) {
        ...
        const cx = bbox.x + bbox.width / 2,
              cy = bbox.y + bbox.height / 2;
        const tlist = getTransformList(elem); // only remove the real rotational transform if present (i.e. at index=0)
      ...
      */
      ...
      const recalculateAllSelectedDimensions = this.recalculateAllSelectedDimensions = function () {
        ...
        const batchCmd = new BatchCommand$1(text);
        let i = selectedElements.length;

        while (i--) {
          const elem = selectedElements[i]; // if (getRotationAngle(elem) && !hasMatrixTransform(getTransformList(elem))) { continue; }
      ...
      */
      ...
      let drawnPath = null; // Mouse events
      ...
      (function () {
        ...
        const getBsplinePoint = function (t) {
          const spline = {
            ...
            y: 0
                ...
                t2 = t * t,
                t3 = t2 * t;
          const m = [[-1, 3, -3, 1], [3, -6, 3, 0], [-3, 0, 3, 0], [1, 4, 1, 0]];
        ...
        const mouseDown = function (evt) {
          ...
          if (mouseTarget === selectorManager.selectorParentGroup && !isNullish(selectedElements[0])) {
            ...
            const griptype = elData(grip, 'type'); // rotating
            ...
            case 'multiselect':
              {
                ...
                for (i = 0; i < len; ++i) {
                  const intElem = newList[i]; // Found an element that was not selected before, so we should add it.
        ...
        const mouseUp = function (evt) {
          ...
          let keep = false;
          const realX = x;
          const realY = y; // TODO: Make true when in multi-unit mode
              ...
              if (!isNullish(selectedElements[0])) {
                ...
                if (realX !== rStartX || realY !== rStartY) {
                  ...
                  if (selectedElements[0].nodeName === 'path' && isNullish(selectedElements[1])) {
                    pathActions.select(selectedElements[0]); // if it was a path
        ...
        function (e) {
          ...
          rootSctm = $$9('#svgcontent g')[0].getScreenCTM().inverse();
          const workarea = $$9('#workarea');
          const scrbar = 15;
          const rulerwidth = curConfig.showRulers ? 16 : 0; // mouse relative to content area in content pixels

          const pt = transformPoint(evt.pageX, evt.pageY, rootSctm); // full work area width in screen pixels

          const editorFullW = workarea.width();
          const editorFullH = workarea.height(); // work area width minus scroll and ruler in screen pixels

          const editorW = editorFullW - scrbar - rulerwidth;
          const editorH = editorFullH - scrbar - rulerwidth; // work area width in content pixels

          const workareaViewW = editorW * rootSctm.a;
          const workareaViewH = editorH * rootSctm.d; // content offset from canvas in screen pixels
          ...
          const topLeftOld = transformPoint(wOffsetLeft, wOffsetTop, rootSctm); // top left of workarea in content pixels after zoom
          ...
          /** @lends module:svgcanvas.SvgCanvas#textActions */
          {
            ...
            */
            init(inputElem) {
              ...
              let i, end; // if (supportsEditableText()) {
              //   curtext.select();
              //   return;
              // }
      ...
      */
      ...
      const removeUnusedDefElems = this.removeUnusedDefElems = function () {
        ...
        const defelems = $$9(defs).find('linearGradient, radialGradient, filter, marker, svg, symbol');
        i = defelems.length;
      ...
      */
      ...
      this.svgCanvasToString = function () {
        ...
        pathActions.clear(true); // Keep SVG-Edit comment on top
        ...
        const nakedSvgs = []; // Unwrap gsvg if it has no special attributes (only id and style)
        ...
        const output = this.svgToString(svgcontent, 0); // Rewrap gsvg
      ...
      this.svgToString = function (elem, indent) {
        ...
        if (elem) {
          ...
          if (elem.id === 'svgcontent') {
            ...
            const res = getResolution();
            const vb = ''; // TODO: Allow this by dividing all values by current baseVal
            ...
            const nsuris = {}; // Check elements for namespaces, add if found
            ...
            while (i--) {
              const attr = attrs[i];
              const attrVal = toXml(attr.value); // Namespaces have already been dealt with, so skip
            ...
            for (let i = attrs.length - 1; i >= 0; i--) {
              const attr = attrs[i];
              let attrVal = toXml(attr.value); // remove bogus attributes added by Gecko
      ...
      */
      ...
      function getIssues() {
        ...
        const issues = [];
        const issueCodes = []; // Selector and notice
        ...
        const content = $$9(svgcontent); // Add font/text check if Canvas Text API is not implemented
      ...
      */
      ...
      this.rasterExport = async function (imgType, quality, exportWindowName, opts = {}) {
        ...
        if (!canvg) {
          ({
            canvg
          } = await importSetGlobal(curConfig.canvgPath + 'canvg.js', {
            global: 'canvg'
          }));
        }
        ...
        const c = $$9('#export_canvas')[0];
        c.width = canvas.contentW;
        c.height = canvas.contentH;
        await canvg(c, svg); // Todo: Make async/await utility in place of `toBlob`, so we can remove this constructor
      ...
      */
      ...
      this.exportPDF = async function (exportWindowName, outputType = isChrome() ? 'save' : undefined) {
        if (!window.jsPDF) {
          // Todo: Switch to `import()` when widely supported and available (also allow customization of path)
          await importScript([// We do not currently have these paths configurable as they are
          ...
          const modularVersion = !('svgEditor' in window) || !window.svgEditor || window.svgEditor.modules !== false; // Todo: Switch to `import()` when widely supported and available (also allow customization of path)
      ...
      */
      ...
      const uniquifyElems = this.uniquifyElems = function (g) {
        const ids = {}; // TODO: Handle markers and connectors. These are not yet re-identified properly
        ...
        walkTree(g, function (n) {
          ...
          if (n.nodeType === 1) {
            ...
            const href = getHref(n); // TODO: what if an <image> or <a> element refers to an element internally?
        ...
        for (const oldid in ids) {
          ...
          if (elem) {
            const newid = getNextId(); // assign element its new id
      ...
      */
      ...
      const convertGradients = this.convertGradients = function (elem) {
        let elems = $$9(elem).find('linearGradient, radialGradient');
        ...
        elems.each(function () {
          ...
          if ($$9(grad).attr('gradientUnits') === 'userSpaceOnUse') {
            ...
            const fillStrokeElems = $$9(svgcontent).find('[fill="url(#' + grad.id + ')"],[stroke="url(#' + grad.id + ')"]');
            ...
            const bb = getBBox(fillStrokeElems[0]); // This will occur if the element is inside a <defs> or a <symbol>,
            ...
            if (grad.tagName === 'linearGradient') {
              const gCoords = $$9(grad).attr(['x1', 'y1', 'x2', 'y2']); // If has transform, convert
      ...
      */
      ...
      const convertToGroup = this.convertToGroup = function (elem) {
        ...
        if ($elem.data('gsvg')) {
          ...
          const prev = $elem.prev(); // Remove <use> element
          ...
          const hasMore = $$9(svgcontent).find('use:data(symbol)').length;
          ...
          if (isGecko()) {
            const dupeGrads = $$9(findDefs()).children('linearGradient,radialGradient,pattern').clone();
      ...
      */
      ...
      this.setSvgString = function (xmlString, preventUndo) {
        try {
          ...
          const batchCmd = new BatchCommand$1('Change Source'); // remove old svg document
          ...
          content.find('image').each(function () {
            ...
            if (val) {
              if (val.startsWith('data:')) {
                ...
                const m = val.match(/svgedit_url=(.*?);/); // const m = val.match(/svgedit_url=(?<url>.*?);/);

                if (m) {
                  const url = decodeURIComponent(m[1]); // const url = decodeURIComponent(m.groups.url);
          ...
          let percs = false; // determine proper size
      ...
      */
      ...
      this.importSvgString = function (xmlString) {
        ...
        try {
          ...
          let useExisting = false; // Look for symbol and make sure symbol exists in image
          ...
          const batchCmd = new BatchCommand$1('Import Image');
          let symbol;
          ...
          if (useExisting) {
            ...
            let svg; // If DOM3 adoptNode() available, use it. Otherwise fall back to DOM2 importNode()
      ...
      */
      ...
      this.setGroupTitle = function (val) {
        ...
        const ts = $$9(elem).children('title');
        const batchCmd = new BatchCommand$1('Set Label');
        let title;
      ...
      */
      ...
      this.setDocumentTitle = function (newTitle) {
        ...
        let docTitle = false,
            oldTitle = '';
        const batchCmd = new BatchCommand$1('Change Image Title');
          ...
          case 'selection':
            {
              ...
              const selectedElems = $$9.map(selectedElements, function (n) {
                if (n) {
                  return n;
                }
      ...
      */
      ...
      const setGradient = this.setGradient = function (type) {
        ...
        let grad = canvas[type + 'Grad']; // find out if there is a duplicate gradient already in the defs

        const duplicateGrad = findDuplicateGradient(grad);
        const defs = findDefs(); // no duplicate found, so import gradient into defs
      ...
      */
      ...
      const findDuplicateGradient = function (grad) {
        const defs = findDefs();
        const existingGrads = $$9(defs).find('linearGradient, radialGradient');
        ...
        while (i--) {
          ...
          if (grad.tagName === 'linearGradient') {
            ...
            const gradAttrs = $$9(grad).attr(radAttrs);
            const ogAttrs = $$9(og).attr(radAttrs);
            let diff = false;
      ...
      */
      ...
      this.setPaint = function (type, paint) {
        // make a copy
        const p = new $$9.jGraduate.Paint(paint);
      ...
      */
      ...
      this.getBlur = function (elem) {
        let val = 0; // const elem = selectedElements[0];
      ...
      (function () {
        ...
        */
        ...
        canvas.setBlur = function (val, complete) {
          ...
          val -= 0;
          const batchCmd = new BatchCommand$1(); // Blur found!
      ...
      */
      ...
      this.setImageURL = function (val) {
        ...
        const attrs = $$9(elem).attr(['width', 'height']);
        const setsize = !attrs.width || !attrs.height;
        const curHref = getHref(elem); // Do nothing if no URL change or size change

        if (curHref === val && !setsize) {
          return;
        }

        const batchCmd = new BatchCommand$1('Change Image URL');
        setHref(elem, val);
        batchCmd.addSubCommand(new ChangeElementCommand$1(elem, {
          '#href': curHref
        }));
        $$9(new Image()).load(function () {
          const changes = $$9(elem).attr(['width', 'height']);
          $$9(elem).attr({
            width: this.width,
            height: this.height
          });
      ...
      */
      ...
      this.setLinkURL = function (val) {
        ...
        if (elem.tagName !== 'a') {
          ...
          const parentsA = $$9(elem).parents('a');
        ...
        const batchCmd = new BatchCommand$1('Change Link URL');
        setHref(elem, val);
        batchCmd.addSubCommand(new ChangeElementCommand$1(elem, {
          '#href': curHref
        }));
      ...
      */
      ...
      const changeSelectedAttributeNoUndo = function (attr, newValue, elems) {
        ...
        let i = elems.length;
        const noXYElems = ['g', 'polyline', 'path']; // const goodGAttrs = ['transform', 'opacity', 'filter'];
      ...
      */
      ...
      this.deleteSelectedElements = function () {
        ...
        for (let i = 0; i < len; ++i) {
          ...
          let t = selected; // this will unselect the element and remove the selectedOutline
      ...
      */
      ...
      this.copySelectedElements = function () {
        const data = JSON.stringify(selectedElements.map(x => getJsonFromSvgElement(x))); // Use sessionStorage for the clipboard data.

        sessionStorage.setItem(CLIPBOARD_ID, data);
        flashStorage();
        const menu = $$9('#cmenu_canvas'); // Context menu might not exist (it is provided by editor.js).
      ...
      */
      ...
      this.pasteElements = function (type, x, y) {
        ...
        if (!len) return;
        const pasted = [];
        const batchCmd = new BatchCommand$1('Paste elements'); // const drawing = getCurrentDrawing();
      ...
      */
      ...
      this.groupSelectedElements = function (type, urlArg) {
        ...
        const batchCmd = new BatchCommand$1(cmdStr); // create and insert the group element
      ...
      */
      ...
      const pushGroupProperties = this.pushGroupProperties = function (g, undoable) {
        ...
        const batchCmd = new BatchCommand$1('Push group properties'); // TODO: get all fill/stroke properties from the group that we are about to destroy
        ...
        const gattrs = $$9(g).attr(['filter', 'opacity']);
        ...
        for (let i = 0; i < len; i++) {
          ...
          let chtlist = getTransformList(elem); // Don't process gradient transforms
          ...
          if (glist.numberOfItems) {
            ...
            if (gangle && glist.numberOfItems === 1) {
              ...
              const coldc = transformPoint(cbox.x + cbox.width / 2, cbox.y + cbox.height / 2, ceqm); // sum group and child's angles
              ...
              const newxform = svgroot.createSVGTransform(); // [ gm ] [ chm ] = [ chm ] [ gm' ]
              // [ gm' ] = [ chmInv ] [ gm ] [ chm ]
      ...
      this.ungroupSelectedElement = function () {
        ...
        if (g.tagName === 'g' || g.tagName === 'a') {
          ...
          while (g.firstChild) {
            ...
            const oldNextSibling = elem.nextSibling;
            const oldParent = elem.parentNode; // Remove child title elements
      ...
      */
      ...
      this.moveUpDownSelected = function (dir) {
        ...
        curBBoxes = [];
        let closest, foundCur; // jQuery sorts this list

        const list = $$9(getIntersectionList(getStrokedBBoxDefaultVisible([selected]))).toArray();
      ...
      this.moveSelectedElements = function (dx, dy, undoable) {
        ...
        while (i--) {
          ...
          if (!isNullish(selected)) {
            ...
            // selectedBBoxes[i] = b;
            const xform = svgroot.createSVGTransform();
            const tlist = getTransformList(selected); // dx and dy could be arrays
      ...
      */
      ...
      this.cloneSelectedElements = function (x, y) {
        let i, elem;
        const batchCmd = new BatchCommand$1('Clone Elements'); // find all the elements selected (stop at first null)
